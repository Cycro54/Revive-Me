    ...
    public static void useOnPlayer(PlayerInteractEvent.EntityInteract event){
        ...
        switch (targCap.getPenaltyType()) {
            case NONE:
                break;
            case HEALTH:
                if(revplayer.getHealth() < targCap.getPenaltyAmount())
                    return;
                break;
            case EXPERIENCE:
                if(revplayer.totalExperience < targCap.getPenaltyAmount())
                    return;
                break;
            case FOOD:
                if (revplayer.getFoodData().getFoodLevel() < targCap.getPenaltyAmount())
                    return;
                break;
        }
